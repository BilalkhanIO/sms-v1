...
const SubjectDetails = () => {
  ...
  const { data: subjectRaw, isLoading, error } = useGetSubjectByIdQuery(id);
  const [deleteSubject, { isLoading: isDeleting }] = useDeleteSubjectMutation();
  const subject = subjectRaw?.data || subjectRaw;

  if (isLoading) return <Spinner size="large" />;
  if (error || !subject) return <div className="text-red-500 p-4">Subject not found.</div>;
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <PageHeader title="Subject Details" backUrl="/dashboard/subjects">
        {can('subjects', 'edit') && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="font-medium">{subject.grade ? `Grade ${subject.grade}` : '—'}</p>
              </div>
            </div>
            <div className="flex items-center space-x-3">
          ...
          <div className="divide-y divide-gray-200">
            {teachers.map((teacher) => (
              <div key={teacher._id} className="px-6 py-4 flex items-center justify-between">
                ...
                <Link
                  to={`/dashboard/teachers/${teacher._id}`}
            ...
            {teachers.length === 0 && (
              <div className="px-6 py-4 text-gray-500">No teachers assigned to this subject.</div>
            )}
          </div>
        </div>
      </div>

      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} title="Delete Subject">
        <div className="p-6">
          <p className="text-gray-600">Are you sure you want to delete this subject? This action cannot be undone.</p>
